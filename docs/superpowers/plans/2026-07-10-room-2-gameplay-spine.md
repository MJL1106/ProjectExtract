# Room 2 Gameplay Spine Implementation Plan
...
Extend the pure tests to verify profile priority as a separate helper: wave profile beats punishment, punishment beats base, removing wave restores punishment, and removing punishment restores base.
...
{
	if (WaveProgress.IsActive() && IsValid(ActiveWaveRequest.ConfigOverride)) return ActiveWaveRequest.ConfigOverride;
	if (PunishmentSource.IsValid() && IsValid(PunishmentConfig)) return PunishmentConfig;
	return Config;
}
```
...
On server BeginPlay, lock the target. On local-player overlap, add the objective. Resolve the overlapping player's `UCompanionCommandComponent`, bind mode changes once, and unlock permanently when the required mode arrives. On unlock: clear the door gate, remove the objective, disable the box, and unbind. Clean bindings in `EndPlay`.
...
Add `ACompanionCharacter::IsShootTakedownArmed() const`, returning armed and active method Shoot. In `AExtractionPlayer::FireStart`, snapshot this state before broadcasting `OnPlayerFiredWeapon`, because the synchronous takedown listener disarms before the actual weapon shot.

Store one bool and expose a non-reflected `ConsumeNextShotStealthExemption()` that returns and clears it. The first actual shot is exempt; continued automatic fire is not.
...
In the authoritative `OnWeaponFiredCallback`, ask the owning `AExtractionPlayer` to consume its one-shot exemption, broadcast, then preserve the existing multicast effects flow.
...
Interaction is accepted once on authority. It removes the reach objective, adds Defend the Position, calls `StartWave`, and binds to completion/blocked events before starting so a synchronous failure cannot be missed.
...
Before unlock, interaction emits **Eliminate remaining enemies** through the existing notification channel. `UnlockExit()` sets the replicated flag once and registers **Use the lift**. After unlock, interaction calls `AExtractionGameMode::CompleteLevel` on authority.
