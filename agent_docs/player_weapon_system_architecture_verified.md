...
|---|---|---|---|
| Procedural FPS Kit | Current player pose, camera, locomotion presentation, ADS, grip IK, rifle/pistol/shotgun action families | `ABP_Manny`, `AC_ProceduralAnimation`, current physical/virtual bones and pose data | Sole player-presentation owner |
| KINEMATION Tactical Shooter | Strong unified camera/weapon/hand presentation, AimPoint/view settings, modular AnimBPs | KIN BPIs/component, `FPCamera`, `VB ik_hand_gun_pivot`, lower-arm/pose/recoil/curve VBs | Source assets now; full replacement only as its own migration |
| Infima Modern Guns | Modular AR/pistol/SMG/shotgun and attachments, meshes/source rigs | No ProjectExtract gameplay/ADS contract; family-specific art rigs | Art/source-rig provider |

KIN's official ADS snaps the weapon to an `FPCamera` socket and applies a weapon-local AimPoint. Its weapons attach to `VB ik_hand_gun_pivot`; its AnimBPs depend on `AC_TacticalShooterAnimation`. The current skeleton and AnimBP do not meet that contract.

KIN migration would require a player-only skeleton or duplicate, KIN VBs/socket, AnimBP integration, interface adapters, camera reconciliation, retargeted traversal/takedown/revive/DBNO, disabling KIN gameplay input/ammo/reload ownership, and new weapon rigging. KIN also has no exact SMG family. Its official Unreal mesh-swap tutorial remains "coming soon," FPS-pack integration is "in progress," and attachment foundation is described as work in progress.
...
Task 6 now has a build-green native ADS runtime:

- Project-owned iron and optic markers resolve to one rigid aim anchor in `ik_hand_gun` space.
- Optics are selected by stable attachment ID, not vendor component-array position.
- Requested optic state is separate from committed state; failed async loads keep a still-valid committed optic or fall back to authored irons.
- Weapon-view teardown invalidates the old sight before publishing the null view/new weapon.
- `UPlayerWeaponPresentationComponent.GetResolvedAimSourceWorld`, `GetResolvedSight`, and `SetSelectedOpticId` are reflected and visible in the reopened editor.
- The legacy kit interface remains at its 30 cm compatibility value. The new adapter must consume the presentation component directly.
- Build log `extraction-build-20260718-101317.out.log` contains `Result: Succeeded`; VibeUE and NeoStack were live after reboot. No PIE was used.

The live kit graph confirmed the scope failure mechanism: `AC_ProceduralAnimation.Found Iron Sights` discovers components by tags and requires exact `FrontAimpoint`, `RearAimpoint`, and `OpticAimpoint` sockets. Multiple `OpticSight`-tagged components can reorder candidates. The project adapter must pass the one resolved world aim anchor to `Calculate Optics` and stop using vendor discovery for profile weapons.

Grip calibration is stored in each kit `DT_ProceduralAnimValues` asset. Rifle, pistol, shotgun, and sniper profiles carry different base/run/sprint transforms; vertical and angled styles use shared pose sequences plus per-profile left-hand corrections. The current `BP_ExtractionCharacter.OnWeaponEquipped` incorrectly forces `ENUM_LeftHand.Vertical` for every weapon. The next native/in-engine adapter task must map the profile's stable grip style to Normal, Vertical, Angled, or no-support-hand.

Reload research confirmed:

- Automatic reload dynamically plays normal/empty arm sequences through `AC_ProceduralAnimation.Play Animation`, then performs vendor ammo arithmetic. ProjectExtract must reuse only the visual playback.
- Shotgun reload is begin/empty-begin plus a one-second shell loop scheduled at sequence length minus 0.15 seconds. `AN_ReloadShotgun` mutates vendor ammo and must not be allowed to author ProjectExtract ammunition.
- `AN_WeaponAnim` hard-casts to the vendor character and drives `SpawnedItem.Item_Mesh`; it cannot drive the project-owned passive view without an adapter.
- Rifle `SpareMag` has no mesh while the actual magazine is a separate `SM_AssaultRifle_Magazine` component, so vendor SpareMag visibility is not a scalable magazine-transfer contract.
- ProjectExtract profiles need paired arms/weapon visual actions, explicit magazine/shell/bolt/pump markers or proxies, and cleanup on interruption, unequip, death, and `EndPlay`. Only native reload phases may mutate ammunition.

Resume at Task 7/9: expose resolved procedural pose and support-hand style from the native component, then wire the profile route in `BP_ExtractionCharacter` while retaining the legacy route for unmigrated weapons. Task 8 reload choreography follows after that adapter contract is live.
