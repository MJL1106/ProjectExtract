...
static UEnvQueryTest_CoverIntent* MakeCoverIntent(UEnvQueryOption* Opt, float MinHostileCoverDist, float MinHostilePawnDist)
{
	// Fallback radii for non-enemy queriers — enemy queriers read their archetype DA at run time.
	UEnvQueryTest_CoverIntent* T = NewObject<UEnvQueryTest_CoverIntent>(Opt);
	T->MinHostileDistance.DefaultValue = MinHostileCoverDist;
	T->MinHostilePawnDistance.DefaultValue = MinHostilePawnDist;
	T->TestPurpose = EEnvTestPurpose::Filter;
	return T;
}
