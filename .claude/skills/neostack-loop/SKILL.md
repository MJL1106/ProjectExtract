...
LUA
bash .claude/skills/neostack-loop/ns.sh "$WD/script.lua"
```
...
| Tools not in ToolSearch after `/mcp` | Don't fight it — drive over curl instead; Claude's MCP client won't reattach a server that was down at launch. |
...
- This skill is for **asset/BP/widget wiring** from the CLI. C++ is still written/built by Claude Code the normal way — never write or compile C++ through the editor MCP.
