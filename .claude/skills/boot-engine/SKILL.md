...
2. **ALWAYS ask the user before closing — one AskUserQuestion, your two answers as the options:**
...
4. **Option B — listen for the restart instead of closing:** another chat (or the user) rebuilds+reboots; that build includes your saved changes too. Launch the watcher in the **background** so the session isn't blocked — it wakes you the moment the editor is back on a build that contains your code:
   ```powershell
   & "C:\Users\matth\Documents\Github\ProjectExtract\.claude\skills\boot-engine\watch-restart.ps1"
   ```
